...
export default function PedidosPage() {
  ...
  async function load() {
    ...
    if (!p) return
    const { data } = await supabase.from('orders').select('*').eq('restaurant_id', p.restaurant_id).order('created_at', { ascending: false })
    setOrders(data || [])
    setLoading(false)
  }
            ...
            {filtered.map(o => (
              <div key={o.id} className="bg-white border border-gray-100 rounded-xl p-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-medium text-gray-900">🪑 {o.table_name}</span>
                  <span className={`badge ${STATUS_CLASS[o.status]}`}>{STATUS_LABEL[o.status]}</span>
                </div>
                <div className="text-xs text-gray-400 mb-2">{new Date(o.created_at).toLocaleTimeString('es-MX',{hour:'2-digit',minute:'2-digit'})}</div>
                <div className="space-y-0.5 mb-3">
                  {(o.items||[]).map((item:string,i:number) => <div key={i} className="text-xs text-gray-600">{item}</div>)}
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-50">
                  ...
                  {o.status !== 'entregado' && (
                    <button onClick={() => advance(o.id, o.status)} className="btn-ghost text-xs">Avanzar →</button>
                  )}
                </div>
              </div>
            ))}
            {filtered.length === 0 && <p className="col-span-3 text-sm text-gray-400 text-center py-8">No hay pedidos.</p>}
          </div>
        )}
      </div>
    </AppShell>
